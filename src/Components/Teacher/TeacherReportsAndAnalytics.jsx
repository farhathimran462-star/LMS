...
const TeacherReportsAnalytics = () => {
  ...
  const teacherUserId = userData?.user_id;
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const { data: classes, error: classError } = await getClassesByTeacher(teacherUserId);
        if (classError) throw new Error(classError.message);
        
        const assignedClassesList = classes || [];
        setAssignedClasses(assignedClassesList);
        ...
        // Fetch report data for assigned classes only
        const { data, error: reportError } = await getAdminReportData({});
        if (reportError) throw reportError;

        // Filter to only show data for assigned classes
        const assignedClassIds = assignedClassesList.map(c => c.id);
        const filteredMarks = (data?.marks || []).filter(m => assignedClassIds.includes(m.class_id));
        const filteredAttendance = (data?.attendance || []).filter(a => assignedClassIds.includes(a.class_id));

        setRawMarksData(filteredMarks);
  ...
  // Get unique institutions from assigned classes
  const instituteOptions = useMemo(() => {
    const uniqueInstituteIds = new Set();
    assignedClasses.forEach(cls => {
      if (cls.institute_id) {
        uniqueInstituteIds.add(cls.institute_id);
      }
    });
    
    // Map institute IDs to institution names from the institutions state
    const institutesList = institutions
      .filter(inst => uniqueInstituteIds.has(inst.id))
      .map(inst => ({
        id: inst.id,
  ...
  }, [rawMarksData, rawAttendanceData, institutions, allCourses, allLevels, allBatches, assignedClasses, allSubjects, allStudents, selectedInstituteId, selectedCourseId, selectedLevelId, selectedBatchId, selectedClassId, selectedSubjectId, selectedStudentId]);
